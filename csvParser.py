...
for iter in range(12):
    if iter<9:
        ...
        iterStr=str(iter+1)
    monthContainer[iterStr]=deepcopy(categoryContainer)
def getExpenses(fileName):
    ...
    with open(fileName) as f:
        csvContents=csv.reader(f,delimiter=';')
    ...
    return expenses
def tidyExpenses(expIn,yearContainer=None):
    ...
    if yearContainer is None: #initialize year container if none is provided as input
        ...
        yearContainer=dict()
        yearContainer[expIn[0][0][0:-4]]=deepcopy(monthContainer)
    else:
    ...
    for exp in expIn:
        ...
        if loopYear not in yearContainer.keys(): #create nested month dictionary if the year is not already in the year container
            yearContainer[loopYear]=deepcopy(monthContainer)
        for word in exp[2].split(' '): #split description string into separate words and check if any of them in the category dictionary
            if word in categoryDict.keys():
                loopCategory=categoryDict[word]
                break
            else:
                loopCategory='Misc'
...
expenses=getExpenses('testfile.csv')
yC=tidyExpenses(expenses)
# print(expenses)
for y in yC.keys():
    print(y)
    for m in yC[y].keys():
